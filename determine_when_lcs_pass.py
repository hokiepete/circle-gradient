...
with hp.File('850mb_300m_10min_NAM_LCS_t=4-215hrs_Sept2017_int=-2.hdf5','r') as loadfile:
    ftle=loadfile['ftle'][:]
...
x, y = m(star[1],star[0])
for radius in [400,1200]:#[200,500,800,1000,2000,3500,5000,7500,10000]:#[200,300,400]:#np.linspace(100,10000,37):#[1,10,100,500,1000,5000,10000,15000]:#radius=1000
    for percent in np.arange(0,101,1):
        ...
        del dirdiv_plot
        plt.close('all')
        np.save('passing_files/passing_times_{0:03d}th_percentile_radius={1:05d}_int=-2'.format(percent,int(radius)),passing_times)
